...
export const SANDBOX_PROVIDER_DEFINITIONS: ReadonlyArray<SandboxProviderDefinition> = [
  SANDBOX_PROVIDER_DEFINITION_BY_ID.e2b,
  SANDBOX_PROVIDER_DEFINITION_BY_ID.daytona,
  SANDBOX_PROVIDER_DEFINITION_BY_ID.vercel,
  SANDBOX_PROVIDER_DEFINITION_BY_ID.upstash,
];

export function sandboxProviderDefinition(
  provider: CloudSandboxProvider,
): SandboxProviderDefinition {
  return SANDBOX_PROVIDER_DEFINITION_BY_ID[provider];
}

function environmentValueIsPresent(
  variable: ProviderInstanceEnvironmentVariable | undefined,
): boolean {
...
export function isSandboxProviderConnected(
  ...
  return sandboxProviderDefinition(provider).fields.every((field) =>
    environmentValueIsPresent(environment.find((variable) => variable.name === field.name)),
  );
}

export function selectableSandboxProviders(
  settings: SandboxSettings,
): ReadonlyArray<SandboxProvider> {
  return [
    "local",
