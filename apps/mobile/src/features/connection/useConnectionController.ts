...
export function useConnectionController() {
  ...
  const connectPairingUrlMutation = useAtomCommand(connectPairingUrlAtom, { reportFailure: false });
  ...
  const updateEnvironment = useCallback(
    ...
    ) => updateBearer({ environmentId, label: updates.label, httpBaseUrl: updates.displayUrl }),
    [updateBearer],
  );
