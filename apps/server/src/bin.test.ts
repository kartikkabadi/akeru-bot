...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect("shows the Akeru Bot root command and server-only serve command", () =>
    Effect.gen(function* () {
      const { output } = yield* captureStdout(runCli(["--help"]));

      assert.include(output, "akeru");
      assert.include(output, "Run the Akeru Bot server.");
      assert.include(output, "serve");
