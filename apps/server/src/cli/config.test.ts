...
const makeDesktopBootstrap = (
  ...
  noBrowser: true,
  port: 4888,
  akeruHome: "/tmp/akeru-bootstrap-home",
  host: "127.0.0.1",
...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
            ...
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  ...
                  T3CODE_PORT: "4001",
                  T3CODE_HOST: "0.0.0.0",
                  AKERU_HOME: join(NodeOS.tmpdir(), "ignored-base"),
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
  ...
  it.effect("uses bootstrap home and ignores the legacy T3CODE_HOME variable", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = "/tmp/akeru-bootstrap-home";
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
