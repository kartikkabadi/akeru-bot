...
import { resolveGitWorktreePath, resolveWorktreeAkeruHome } from "./devHome.ts";
...
describe("resolveWorktreeAkeruHome", () => {
  it.effect("answers with .akeru before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeAkeruHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".akeru"));
